...
export function getCollisionType(collision) {
  ...
  const types = {
    ...
    0x10: { type: 'PASSABLE', walkable: true, surfable: false, description: 'Passable ground (flower/decoration)' },
    0x11: { type: 'WALL', walkable: false, surfable: false, description: 'Wall variant' },
    ...
    0x48: { type: 'WATER', walkable: false, surfable: true, description: 'Water variant (surfable)' },
    ...
    0x19: { type: 'LEDGE_UP', walkable: true, surfable: false, description: 'Ledge (jump up)' },
    0x1A: { type: 'LEDGE_DOWN', walkable: true, surfable: false, description: 'Ledge (jump down) variant' },
    ...
    // Decorative blocking elements (corners, fence posts, signs)
    0x37: { type: 'WALL', walkable: false, surfable: false, description: 'Corner/decorative blocking tile' },
    0x38: { type: 'WALL', walkable: false, surfable: false, description: 'Corner/decorative blocking tile' },
    0x39: { type: 'WALL', walkable: false, surfable: false, description: 'Corner/decorative blocking tile' },
    0x3A: { type: 'WALL', walkable: false, surfable: false, description: 'Corner/decorative blocking tile' },
    0x3B: { type: 'WALL', walkable: false, surfable: false, description: 'Corner/decorative blocking tile' },
    
    // Additional wall types
    0x1B: { type: 'WALL', walkable: false, surfable: false, description: 'Wall variant' },
    0x21: { type: 'WALL', walkable: false, surfable: false, description: 'Wall variant' }
  };
  ...
  // Classify unknown values based on Pokemon Red collision system:
  // - Values 0x00-0x0F: Walls and blocking tiles (except 0x00 which is passable)
  // - Values 0x10-0x1F: Various walls and blocking tiles
  // - Values 0x20+: Generally passable tiles with special properties
  
  if (collision >= 0x01 && collision < 0x20) {
    // Blocking tiles
    ...
    // Passable tiles with unknown special properties
    return { 
      type: 'PASSABLE', 
      walkable: true, 
      surfable: false, 
      description: `Passable tile 0x${collision.toString(16).toUpperCase()}` 
    };
  }
