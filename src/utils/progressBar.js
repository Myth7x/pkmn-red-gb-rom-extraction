 ...
 * Progress Bar Module
 * 
 * Provides a customizable console progress bar for tracking extraction operations.
 * Features include percentage display, ETA calculation, success/error counters,
 * dynamic status messages, and formatted output with Unicode box-drawing characters
 * for professional terminal output during long-running operations.
 */
...
class ProgressBar {
  ...
  error(item = '', errorMsg = '') {
    ...
    console.log(`\x1b[31m[ERROR]\x1b[0m ${item}${errorMsg ? ` - ${errorMsg}` : ''}`);
