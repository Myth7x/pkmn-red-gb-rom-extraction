...
export class MapViewer {
    constructor(canvasId, mode = 'map-viewer') {
        ...
        // State management
        this.preferences = new PreferencesManager(this.config); // Pass config for mode awareness
        this.viewportState = new ViewportState();
        this.mapState = new MapState(); // No longer needs preferences
        
        // Game mode components (only initialized in game mode)
        this.player = null;
        this.camera = null;
        ...
        if (this.config.isGameMode()) {
            ...
            this.tileOptimizationEnabled = true; // Enabled for map-viewer
        }
    ...
    async initGameMode() {
        ...
        this.viewportState.setScale(savedZoom); // Also set viewport scale for consistency
        
        // Mouse interaction state for game mode
        this.mouseHoverTile = null; // {x, y} in player units
        
        Logger.success('Game mode initialized');
    }
    
    /**
     * Initialize town navigation selector for game mode
     * @param {Array} maps - Array of all maps
     */
    initTownNavigation(maps) {
        const townSelect = document.getElementById('townSelect');
        const navigateBtn = document.getElementById('navigateToTownBtn');
        ...
        towns.forEach(town => {
            ...
            option.value = town.mapId;
            option.textContent = town.name;
            townSelect.appendChild(option);
        });
        
        // Enable button when town is selected
        townSelect.addEventListener('change', () => {
            navigateBtn.disabled = !townSelect.value;
        });
        
        // Handle navigation button click
        navigateBtn.addEventListener('click', () => {
            const selectedMapId = parseInt(townSelect.value);
            if (!isNaN(selectedMapId)) {
    ...
    /**
     * Navigate player to a town using pathfinding
     * @param {number} targetMapId - ID of the target map
     */
    async navigateToTown(targetMapId) {
        try {
            const currentMap = this.mapState.currentMap;
            
            if (!currentMap || !this.player) {
                Logger.error('Cannot navigate: No current map or player');
                return;
            }
    ...
    /**
     * Handle warp trigger - load destination map and place player at warp destination
     * @param {Object} warp - Warp data {mapId, destWarpId, x, y}
     */
        ...
        try {
            ...
            if (warp.mapId === 255) {
                ...
                if (sourceMap) {
                    ...
                    Logger.success(`✅ Returned to ${destMap.name} at (${sourceMap.x}, ${sourceMap.y})`);
                    return;
                } else {
                    Logger.error('No source overworld map found for map 255 warp!');
                    return;
                }
            }
            
            // Load the destination map without spawning player
            await this.loadMap(targetMapId, { skipPlayerSpawn: true });
            
            // Find the destination warp in the new map
            const destMap = this.mapState.currentMap;
            if (!destMap || !destMap.objects || !destMap.objects.warps || !destMap.objects.warps.data) {
                Logger.error('Destination map has no warps!');
                return;
            }
            
            // Find the warp with matching warpId (this is the destination warp)
            // In Pokemon Red, warps are paired - the warpId indicates which warp in the destination map
            const destWarps = destMap.objects.warps.data;
            let destWarp = null;
            
            // The warpId is an index into the destination map's warp array
            if (targetWarpId >= 0 && targetWarpId < destWarps.length) {
                destWarp = destWarps[targetWarpId];
            }
            
            if (!destWarp) {
                Logger.error(`Destination warp ${targetWarpId} not found in map ${targetMapId}!`);
                return;
            }
            ...
            Logger.success(`✅ Warped to ${destMap.name} at (${spawnX}, ${spawnY})`);
            
        } catch (error) {
            Logger.error(`Failed to warp: ${error.message}`);
            console.error(error);
        }
    }
    ...
    async handleBoundaryTransition(boundaryData) {
        try {
            ...
            // alignment = offset * -2 (from pret/pokered), so offset = -alignment / 2
            let newPlayerX = playerX;
            let newPlayerY = playerY;
            
            const currentMapWidthPlayerUnits = currentMap.width * 2;
            const currentMapHeightPlayerUnits = currentMap.height * 2;
            const newMapWidthPlayerUnits = newMap.width * 2;
            const newMapHeightPlayerUnits = newMap.height * 2;
            
            // Get the map rendering offset (in blocks)
            // This is how the connected map is positioned relative to the current map
            const mapOffsetX = -connection.xAlignment / 2; // In blocks
            const mapOffsetY = -connection.yAlignment / 2; // In blocks
            
            // Convert to player units (blocks * 2)
            const mapOffsetXPlayerUnits = mapOffsetX * 2;
    ...
    async getVisibleConnectedMaps() {
        ...
        const addConnectedMap = async (direction, baseOffsetX, baseOffsetY) => {
            ...
            // Calculate connected map position using ROM alignment values
            // The alignment values (_x, _y) are calculated from the offset parameter as: alignment = offset * -2
            // So to get the rendering offset: offset = -alignment / 2
            // The offset is in BLOCKS (4x4 tiles = 32x32 pixels)
            let offsetX = baseOffsetX;
            let offsetY = baseOffsetY;
            
            if (direction === 'north') {
                // Place connected map above current map
    ...
    async render() {
        ...
        try {
            ...
            let scale = this.viewportState.getScale();
            let offset = this.viewportState.getOffset();
            
            // In game mode, use camera offset and zoom instead
            if (this.config.isGameMode() && this.camera) {
                const cameraOffset = this.camera.getOffset();
                offset = { x: cameraOffset.offsetX, y: cameraOffset.offsetY };
                scale = this.camera.getZoom(); // Use camera's zoom level
            }
        ...
        // Render overlays (warps, signs, NPCs) - only for main map and only in map-viewer mode
        if (this.showOverlays && this.config.isFeatureEnabled('overlays')) {
        ...
        if (this.config.isGameMode() && this.player && this.camera) {
            ...
            if (this.mouseHoverTile) {
                this.renderHoverTile(this.mouseHoverTile, cameraOffset.offsetX, cameraOffset.offsetY, scale);
            }
        }
        
        // Draw grid if enabled (only in map-viewer mode)
        if (this.showGrid && this.config.isFeatureEnabled('grid')) {
    ...
    /**
     * Render hover tile indicator in game mode
     * Shows a highlighted border around the destination tile (2x2 tiles = 16x16 pixels)
     * @param {Object} hoverTile - {x, y} in player units
     * @param {number} offsetX - Camera offset X
     * @param {number} offsetY - Camera offset Y
     * @param {number} scale - Zoom scale
     */
    renderHoverTile(hoverTile, offsetX, offsetY, scale) {
        if (!hoverTile) return;
        ...
        const borderWidth = Math.max(2, scale * 1.5);
        this.ctx.save();
    ...
    /**
     * Render walking path in game mode
     * Shows transparent dots on tiles the player will walk through
     ...
     * @param {number} scale - Zoom scale
     */
    renderWalkingPath(path, currentIndex, offsetX, offsetY, scale) {
        if (!path || path.length === 0) return;
        
        const tileSize = 16; // Player units are 16 pixels
        
        // Path color (cyan/blue - same as hover effect for walkable tiles)
        const pathColor = 'rgba(100, 200, 255, 0.4)';
        const destinationColor = 'rgba(100, 200, 255, 0.6)'; // Slightly more opaque for destination
        
        // Draw remaining path (from current index to end)
        for (let i = currentIndex; i < path.length; i++) {
            ...
            const dotColor = isDestination ? destinationColor : pathColor;
            
            // Draw dot in center of tile
            const centerX = screenX + size / 2;
            const centerY = screenY + size / 2;
            const dotRadius = Math.max(3, scale * 2); // Scale with zoom
            ...
            // For destination, also draw a ring around the dot
            if (isDestination) {
                this.ctx.save();
                this.ctx.strokeStyle = 'rgba(100, 200, 255, 0.8)';
                this.ctx.lineWidth = Math.max(1.5, scale * 0.75);
                this.ctx.beginPath();
                this.ctx.arc(centerX, centerY, dotRadius + Math.max(2, scale), 0, Math.PI * 2);
                this.ctx.stroke();
                this.ctx.restore();
            }
        }
        ...
        if (currentMap.objects && currentMap.objects.sprites && currentMap.objects.sprites.data && currentMap.objects.sprites.data.length > 0) {
            ...
            currentMap.objects.sprites.data.forEach((sprite, index) => {
                ...
                if (spriteImg && spriteImg.complete && spriteImg.naturalWidth > 0) {
                    ...
                    // Draw overlay indicator on top if overlays are enabled (map-viewer mode only)
                    if (this.showOverlays && this.config.isFeatureEnabled('indicators')) {
                    ...
                    // Fallback to indicator if sprite not loaded (only in map-viewer mode)
                    if (this.config.isFeatureEnabled('indicators')) {
                        this.renderer.setAlpha(0.5);
    ...
    setupMouseControls() {
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            if (this.isRoomDragging && this.draggedRoom) {
                ...
                // Game mode: Update hover tile for destination visualization
                if (this.config.isGameMode() && this.player && this.camera) {
                    this.handleGameModeMouseMove(e);
                } else {
        ...
        this.canvas.addEventListener('mouseup', (e) => {
            ...
            if (!hasDragged) {
                // Game mode: Handle click to move
                if (this.config.isGameMode() && this.player && this.camera) {
                    this.handleGameModeClick(e);
        ...
        this.canvas.addEventListener('mouseleave', () => {
            ...
            // Clear game mode hover tile
            if (this.config.isGameMode() && this.mouseHoverTile) {
    ...
    /**
     * Handle mouse movement in game mode
     * Update hover tile for destination visualization
     */
    handleGameModeMouseMove(e) {
        const currentMap = this.mapState.getCurrentMap();
        if (!currentMap) return;
    ...
    /**
     * Handle mouse click in game mode
     * Find path and move player to clicked tile
     */
    handleGameModeClick(e) {
        const currentMap = this.mapState.getCurrentMap();
        if (!currentMap || !this.player || !this.pathfinding) return;
...
${sign.scriptText.hexString}
    ...
    zoomIn() {
        if (this.viewportState.zoomIn(this.renderer.getWidth(), this.renderer.getHeight())) {
            // Save zoom to appropriate storage based on mode
            if (this.config.isGameMode()) {
                this.preferences.saveGameZoom(this.viewportState.getScale());
                // Also update camera zoom
                if (this.camera) {
                    this.camera.setZoom(this.viewportState.getScale());
                }
            } else {
    ...
    async loadMapList() {
        try {
            ...
            if (this.config.isGameMode()) {
                const savedState = this.preferences.loadGameState();
                ...
                if (savedState && savedState.mapId !== null) {
                    ...
                    if (this.player) {
                        const currentMap = this.mapState.currentMap;
                        this.player.spawn(savedState.playerX, savedState.playerY, currentMap.mapId, currentMap.name, this.preferences, savedState.facing);
                        this.camera.focusOnPlayer(this.player.state.pixelX, this.player.state.pixelY);
                        this.camera.setPosition(this.camera.targetOffsetX, this.camera.targetOffsetY);
                    }
                } else {
                ...
                // Initialize town navigation for game mode
                this.initTownNavigation(data.maps);
            ...
            // Load saved or first map (map-viewer mode only, game mode handled above)
            // Map-viewer mode: Restore last viewed map
    ...
    async loadMap(mapId, options = {}) {
        const { suppressErrorUI = false, skipPlayerSpawn = false } = options;
        ...
        try {
            ...
            // Save map state based on mode
            if (this.config.isGameMode()) {
                // Game mode: Don't save map yet, wait for player spawn
                // (will be saved after spawn to include player position)
            } else {
                // Map-viewer mode: Save to map-viewer storage
                this.preferences.saveCurrentMap(mapId);
            }
            
            // Save previous map ID to localStorage (mode-specific)
            if (this.mapState.previousMapId !== null) {
                if (this.config.isGameMode()) {
                    this.preferences.saveGamePreviousMap(this.mapState.previousMapId);
                } else {
            ...
            // Spawn player in game mode (unless skipPlayerSpawn is true)
            if (this.config.isGameMode() && this.player && this.camera && !skipPlayerSpawn) {
                // Spawn player at center of map, 2 blocks down (under the door)
                // Player position is in 16-pixel units (player sprite size = 16x16)
                // Each block is 32 pixels = 2 player units
                const spawnBlockX = Math.floor(mapData.width / 2);
                const spawnBlockY = Math.floor(mapData.height / 2) + 2;
                const spawnX = spawnBlockX * 2; // Convert blocks to player units (32 pixels / 16 = 2)
                const spawnY = spawnBlockY * 2; // Convert blocks to player units
                
                this.player.spawn(spawnX, spawnY, mapData.mapId, mapData.name, this.preferences);
                
                // Set camera map boundaries and zoom
